...
Module lua
  ...
  Procedure openModLua(L, file$)
    ...
    If lua_pcall(L, 0, 1, 0) <> 0 ; call with 0 arguments and 1 result expected
      debugger::add("lua::openModLua() - lua_pcall ERROR: "+PeekS(lua_tostring(L, -1), -1, #PB_UTF8))
      lua_pop(L, 1)
      ProcedureReturn #False
    EndIf
  ...
  Procedure openStringsLua(L, file$)
    ...
    ; call data()
    If lua_pcall(L, 0, 1, 0) <> 0 ; call with 0 arguments and 1 result expected
      debugger::add("lua::openStringsLua() - lua_pcall ERROR: "+PeekS(lua_tostring(L, -1), -1, #PB_UTF8))
      lua_pop(L, 1)
      ProcedureReturn #False
    EndIf
