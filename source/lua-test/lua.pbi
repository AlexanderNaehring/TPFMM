...
ImportC ""
	...
	lua_pushinteger(L,n)
	lua_pushlstring(L,string.p-ascii,sl)
	lua_pushstring(L,string.p-ascii)
	lua_pushvfstring(L,string.p-ascii,*arg)
	;lua_pushfstring(L,string.p-ascii, ...)
	lua_pushcclosure(L,fn.l ,n)
	lua_pushboolean(L,b)
	lua_pushlightuserdatas(L,p)
	lua_pushthread(L)
	lua_gettable(L,idx)
	lua_getfield(L,idx,string.p-ascii)
	lua_rawget(L,idx)
	lua_rawgeti(L,idx,n)
	...
	lua_getfenv(L,idx)
	lua_settable(L,idx)
	lua_setfield(L,idx,string.p-ascii)
	lua_rawset(L,idx)
	lua_rawseti(L,idx,n)
	...
	lua_load(L,reader,dt,string.p-ascii)
	...
	lua_getinfo (L,string.p-ascii, lua_Debugar)
...
ImportC ""
	luaL_getn(L,t)
	luaL_setn(L,t,n)
	luaI_openlib(L,libname.p-ascii,rl,nup)
	luaL_register(L,libname.p-ascii,rl)
	luaL_getmetafield(L,obj,e.p-ascii)
	luaL_callmeta(L,obj,e.p-ascii)
	luaL_typerror(L,narg,tname.p-ascii)
	luaL_argerror(L,numarg,extramsg.p-ascii)
	luaL_checklstring(L,numArg,sl)
	luaL_optlstring(L,numArg,def.p-ascii,sl)
	luaL_checknumber.d(L,numArg)
	luaL_optnumber.d(L,nArg,def.d)
	luaL_checkinteger(L,numArg)
	luaL_optinteger(L,nArg,def)
	luaL_checkstack(L,sz,msg.p-ascii)
	luaL_checktype(L,narg,t)
	luaL_checkany(L,narg)
	luaL_newmetatable(L,tname.p-ascii)
	luaL_checkudata(L,ud,tname.p-ascii)
	luaL_where(L,lvl)
	;luaL_error(L, const char *fmt, ...)
	luaL_checkoption(L,narg,def.p-ascii,*lst)
	luaL_ref(L,t)
	luaL_unref(L,t,ref)
	luaL_loadfile(L,filename.p-ascii)
	luaL_loadbuffer(L,buff.p-ascii,sz,name.p-ascii)
	luaL_loadstring(L,s.p-ascii)
	luaL_newstate()
	luaL_gsub(L,s.p-ascii,p.p-ascii,r.p-ascii)
	luaL_findtable(L,idx,fname.p-ascii,szhint)
	luaL_buffinit(L, b)
	luaL_prepbuffer(b)
	luaL_addlstring(b,s.p-ascii,l)
	luaL_addstring(b,s.p-ascii)
	luaL_addvalue(b)
	luaL_pushresult(b)
EndImport
